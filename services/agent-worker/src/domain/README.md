...
현재 이 디렉터리에서 Claude Agent SDK를 호출하는 도메인 에이전트는 Chat·Recipe·Cleanup·Title이다. 계약의 잡 종류는 `title.suggestion`·`recipe.scan`·`task.cleanup` 셋이며 `support/job.const.ts`가 그 셋을 그대로 든다.

...
import type { CleanupTaskReaderPort } from "../port/cleanup.task.reader.port.js";
...
export class PrepareTaskCleanupUsecase {
    constructor(
        ...
        private readonly tasks: CleanupTaskReaderPort,
    ...
    async execute(input: TaskCleanupInput): Promise<TaskCleanupPrep> {
        ...
        const language = normalizeOutputLanguage(
            await this.settings.findValue(job.userId, CLEANUP_SETTING_KEY.outputLanguage),
        );
        // 예산과 턴과 마감은 잡이 하는 일의 크기에서 나오므로 모델을 바꿔도 이 기능이 그대로 갖는다.
        const model = chosenJobModel(await this.settings.findValue(job.userId, CLEANUP_SETTING_KEY.anthropicModel), AGENT.taskCleanup.id);
        ...
        if (this.agent.requiresLocalApiKey()) {
            const apiKey = await this.settings.findValue(job.userId, CLEANUP_SETTING_KEY.anthropicApiKey);
        ...
        const batch = await this.tasks.loadScanBatch(job.userId);
    ...
    private async resolveMaxSuggestions(userId: string, requested: number | undefined): Promise<number> {
        ...
        const raw = await this.settings.findValue(userId, CLEANUP_SETTING_KEY.maxSuggestions);

...
import { keepOneInspectPerTask } from "./cleanup.sdk.orchestration.js";
...
describe("한 실행이 조사할 태스크 명단", () => {
    it("태스크가 서로 다르면 모두 조사한다", () => {
        const kept = keepOneInspectPerTask([assignment("task-1"), assignment("task-2")]);

        expect(kept.map(({ taskId }) => taskId)).toEqual(["task-1", "task-2"]);
    });

    it("모델이 같은 태스크를 겹쳐 내면 먼저 적은 것만 남긴다", () => {
        const kept = keepOneInspectPerTask([assignment("task-1", "deep"), assignment("task-1", "shallow")]);
    ...
    it("겹친 태스크를 걸러도 다른 태스크는 그대로 남긴다", () => {
        const kept = keepOneInspectPerTask([
            assignment("task-1"),
            assignment("task-2"),
            assignment("task-1"),
        ]);
    ...
    it("빈 명단은 그대로 빈 채로 낸다", () => {
        expect(keepOneInspectPerTask([])).toEqual([]);
    });
});

...
대상이 `recipe`인 행 하나로 남고 `agent-api`의 배출기가 뒤에서 반영한다. 그 행의 `backend`에는
이 축의 상수가 적히고 배출기는 자기 축이 적재한 행만 가져간다. 같은 `sourceJobId`로 적힌 후보가
이미 있으면 아무것도 쓰지 않으므로 같은 잡의 재시도가 후보를 두 벌 만들지 않는다.

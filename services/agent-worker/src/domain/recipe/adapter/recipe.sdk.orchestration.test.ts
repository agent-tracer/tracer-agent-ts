...
import { keepOneProbePerAxis } from "./recipe.sdk.orchestration.js";
...
describe("한 라운드가 세우는 전문가 명단", () => {
    it("축이 서로 다르면 모두 세운다", () => {
        const kept = keepOneProbePerAxis([assignment("web", "무엇을"), assignment("code", "어디서")], 0);

        expect(kept.map(({ probe }) => probe)).toEqual(["web", "code"]);
    });

    it("모델이 같은 축을 겹쳐 내면 먼저 적은 것만 남긴다", () => {
        const kept = keepOneProbePerAxis(
    ...
    it("빈 명단은 그대로 빈 채로 낸다", () => {
        expect(keepOneProbePerAxis([], 0)).toEqual([]);
    });
});

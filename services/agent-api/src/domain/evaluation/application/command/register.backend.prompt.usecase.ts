...
import { PromptChannelAssignment, PromptDefinition, PromptVersion, type PromptBackend, type PromptVersionInput } from "~agent-api/domain/evaluation/model/prompt.model.js";
...
@Injectable()
export class RegisterBackendPromptUseCase {
    ...
    async execute(input: { userId: string; backend: PromptBackend; agentName: string; language: string; name: string; version: PromptVersionInput }) {
        const now = this.clock.now();
        const definition = Object.assign(new PromptDefinition(), { id: this.ids.next("prompt"), userId: input.userId,
            agentName: input.agentName, backend: input.backend, language: input.language, name: input.name, createdAt: now });
        ...
        await this.repository.registerBackendPrompt(input.userId, definition, version, channel);

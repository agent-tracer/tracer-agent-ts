...
describe("RegisterPythonPromptUseCase", () => {
    it.each(cases.map((entry) => [entry.name, entry] as const))(
        ...
        async (_name, entry) => {
            const { version } = await new RegisterPythonPromptUseCase(new InMemoryPromptRepository(),
                new SequentialPromptIdGenerator(), new FixedPromptClock(new Date(0)))
                .execute({ userId: "local", agentName: "task-cleanup", language: "en", name: "investigator",
    ...
    it("요청이 실은 해시가 규칙과 다르면 거절한다", async () => {
        await expect(new RegisterPythonPromptUseCase(new InMemoryPromptRepository(),
            new SequentialPromptIdGenerator(), new FixedPromptClock(new Date(0)))
            .execute({ userId: "local", agentName: "task-cleanup", language: "en", name: "investigator",

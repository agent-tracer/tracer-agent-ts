...
/** 한 도구의 계약이며, required·optional·mutation은 예약된 키이고 그 밖의 키는 인자의 제약이다. */
...
export interface ChatToolContract {
    ...
    /** 도구가 무너졌을 때 모델이 읽는 문구이며 자리를 읽는 쪽이 채운다. */
    readonly failures: { readonly toolFailed: string };
    /** 도구 이름별 인자 이름별 설명이며 두 구현체가 같은 문장을 인자 스키마에 싣는다. */
...
export const CHAT_TOOL_CONTRACT: ChatToolContract =
    readContractJson<ChatSpec>("agent/chat/spec.json").tools;
...
/** 도구 이름별 zod shape이며, 실행자가 같은 shape로 인자를 파싱하고 좁힌다. */

...
const CONSTRAINT = "ai_job_steps_job_attempt_seq";
...
function step(overrides: Partial<JobStepEntity> = {}): JobStepEntity {
    const row = new JobStepEntity();
    row.id = "step-1";
    row.jobId = "job-1";
    row.userId = "user-1";
    row.attempt = 1;
...
beforeAll(async () => {
    ledger = await startLedger(path.join(CONTRACT_ROOT, "db", "migrations"), [JobStepEntity]);
...
describe("한 시도 안에서 잡 궤적의 순번은 하나다", () => {
    it("같은 잡과 시도와 순번의 두 번째 단계를 거절한다", async () => {
        const rows = ledger.repository(JobStepEntity);
    ...
    it("다른 잡은 같은 시도와 순번을 쓴다", async () => {
        const rows = ledger.repository(JobStepEntity);
        await rows.save(step());

        await expect(rows.save(step({ id: "step-2", jobId: "job-2" }))).resolves.toBeDefined();
    });
});

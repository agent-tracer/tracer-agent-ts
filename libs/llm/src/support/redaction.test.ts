...
describe("값을 견주는 절차", () => {
    ...
    it("유니코드로 접으면 길이가 달라지는 글자가 앞에 있어도 자리가 밀리지 않는다", () => {
        for (const word of ["\u0130stanbul", "Stra\u00dfe"]) {
            expect(redactText(`${word} 에서 sk-ant-${BODY} 를 받았다`)).toBe(`${word} 에서 ${RULE.marker} 를 받았다`);
        }
    });
